class LinkUrlResourcesController < ApplicationController
  ...
  def create
    ...
    if !params[:resource][:title].blank? and !params[:resource][:keywords].blank? and !flash[:link_url].blank?
      @link_url_resource.update_attributes(:resource_type => "link_url_resource",:title=>params[:resource][:title], :keywords=>params[:resource][:keywords],:link_url => flash[:link_url])
