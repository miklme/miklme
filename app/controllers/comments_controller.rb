  ...
  def create
    ...
    @comment.owner=current_user
      ...
      if not u==current_user
        if current_user.field_value(@resource.keyword_page)<0 and params[:comment][:rating]=="-"
          flash[:notice]="这个领域内你的价值点数低于0了，暂时不能作出这种评价。"
        elsif current_user.field_value(@resource.keyword_page)>=0 and params[:comment][:rating]=="-"
