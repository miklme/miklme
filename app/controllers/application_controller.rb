...
class ApplicationController < ActionController::Base
  ...
  def auto_complete_for_keyword_page_keyword
    ...
    @keyword_pages=keyword_pages.to_a.find_all{|k| k.resources.size>=1}.first(15)
    render :layout => false
  end
  ...
  def comment_notice(comment)
    ...
    if comment.changed_value<0 and comment.rating<0
      "你试图攻击#{comment.parent.owner.nick_name}，可是对方声望比你高，你被反弹，声望<cite>#{comment.changed_value}</cite>"
      #比别人差，但支持别人
