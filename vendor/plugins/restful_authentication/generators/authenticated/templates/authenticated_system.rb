  ...
  def current_<%= file_name %>=(new_<%= file_name %>)
    ...
    # Called from #current_<%= file_name %>.  Finaly, attempt to login by an expiring token in the cookie.
      ...
      def logout_keeping_session!
        ...
        @current_file_name.forget_me if @current_file_name.is_a? File_name
        @current_file_name= false     # not logged in, and don't do it for me
        kill_remember_cookie!     # Kill client-side auth cookie
        session[:file_name_id] = nil   # keeps the session but kill our variable
